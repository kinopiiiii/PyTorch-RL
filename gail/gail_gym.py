...
def update_params(batch, i_iter):
    ...
    for _ in range(3):
        ...
        if DEBUG:
            # print("expert_traj",expert_traj)
            # print("expert_state_actions",expert_state_actions)
            # print("torch.cat([states, actions], 1)", torch.cat([states,actions], 1))
            # print("Variable(torch.cat([states, actions], 1))", Variable(torch.cat([states, actions], 1)))
            # print("g_o", g_o)
            # print("ones((states.shape[0], 1)",ones((states.shape[0], 1)))
